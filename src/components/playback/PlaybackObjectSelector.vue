...
const props = defineProps(["playbackService"])
        ...
        <Listbox v-model="selectedCountry" :options="getDisplayingObjects" optionLabel="identifier" class="w-full" listStyle="max-height:250px">
            <template #option="slotProps">
                <div class="w-full flex flex-row gap-[15px] items-center">
                    ...
                    <Button icon="pi pi-times" size="small" class="ml-auto !p-0" variant="text" severity="danger" @click="removeDisplayingObject(slotProps.option[0])" />
                </div>
            </template>
        </Listbox>
    </div>
