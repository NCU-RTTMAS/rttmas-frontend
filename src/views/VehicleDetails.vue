...
export default defineComponent({
  ...
  methods: {
    async fetchData() {
      try {
        const response = await apiService.getVehicleById(this.vehicle.plate);
        console.log(response.data)
        this.data = response.data;
        this.data.pathHistory = response.data.tracks
      } catch (error) {
  ...
  mounted() {
    this.fetchData().then(()=> {
      ...
      this.pathHistory = this.data["tracks"]
    });
...
<template>
  <main>
    <div class="w-full p-10 flex flex-col gap-[30px]">
      ...
      <section>
        <h1 class="text-2xl font-bold">{{ vehicle.plate }}</h1>
      ...
      <section>
        ...
        <table class="mt-[10px] w-full table-auto">
          ...
          <tr v-for="record in pathHistory">
            <td>{{ (new Date(record.time * 1000 )).toLocaleString("EN-ZA") }}</td>
            <td>{{ record.lat }}</td>
            <td>{{ record.lon }}</td>
          </tr>
        </table>
